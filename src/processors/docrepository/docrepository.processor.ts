...
import { getGitFileCreationDate } from '../../utils/files.js'
    ...
    implements ApicuronProcessor<DocRepositoryProcessorInput>
{
    ...
    async process(input: DocRepositoryProcessorInput): Promise<Report[]> {
        ...
        if (contributions.length > 0) {
            ...
            for (const { filePath, contributors } of contributions) {
                const { reports, missingOrcids } = await this.processOneArticle(
                    {
                        filepath: filePath,
    ...
    async processOneArticle({
        ...
        // const mtimestamp = await getFileModificationDate(filepath)
        // const ctimestamp = await getfilecreationDate(filepath)
        const gitCreationTimestamp = await getGitFileCreationDate(filepath)
        ...
        const reports = orcids.map((orcid) => ({
            ...
            timestamp: gitCreationTimestamp.toISOString()
        }))
